...
def test_normalize_flags_created_container_as_stopped() -> None:
    container = _normalize_container(
        {
            "Names": ["/correlcore-worker"],
            "State": "created",
...
def test_list_stack_containers_uses_compose_project_not_name_prefix(monkeypatch) -> None:
    from app.services import docker_health_service
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            {
                "Names": ["/correlcore-test-worker"],
                "State": "exited",
                "Status": "Exited (1) 1 second ago",
                "Labels": {
                    "com.docker.compose.project": "correlcore-test",
            ...
            },
            {
                ...
                "Labels": {
                    "com.docker.compose.project": "correlcore",
                    "com.docker.compose.service": "worker",
                },
            },
        ],
    ...
    containers = list_stack_containers()
    assert [item.name for item in containers] == ["correlcore-worker"]


def test_list_stack_containers_respects_compose_project_name(monkeypatch) -> None:
    from app.services import docker_health_service

    monkeypatch.setenv("COMPOSE_PROJECT_NAME", "correlcore-test")
    monkeypatch.setattr(
        docker_health_service,
        "_load_raw_containers",
        lambda: [
    ...
    containers = list_stack_containers()
    assert [item.name for item in containers] == ["correlcore-test-worker"]
...
def test_cli_fallback_skipped_in_production(monkeypatch) -> None:
    from app.services import docker_health_service

    monkeypatch.setattr(docker_health_service.settings, "DEV_DOCKER_HOST", "")
    monkeypatch.setattr(docker_health_service.settings, "APP_ENV", "production")
    monkeypatch.setattr(docker_health_service.shutil, "which", lambda _name: "/usr/bin/docker")
    assert list_stack_containers() == []
