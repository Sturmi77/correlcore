...
async def test_generate_llm_statement_disabled_makes_no_http_call() -> None:
    """When LLM is off, return None without importing/calling httpx."""
    with patch.object(settings, "INSIGHTS_LLM_ENABLED", False):
        # Patch the global module: llm_statements imports httpx lazily inside
        # the enabled path only, so the attribute is not on the service module.
