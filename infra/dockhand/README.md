...
| `MINIO_SECRET_KEY` | **ja** (bei `APP_ENV=production`) | `CHANGE_ME_MINIO_SECRET` | Placeholder bis M13 (Fotos). Production-Settings lehnen `CHANGE_ME*` und Secrets &lt; 16 Zeichen ab. Compose reicht den Wert an `migrate`/`api`/`worker` durch — nur in der `.env` setzen reicht nicht ohne Compose-Mapping. Generieren: `python -c 'import secrets; print(secrets.token_urlsafe(24))'`.                                                                                                    |
