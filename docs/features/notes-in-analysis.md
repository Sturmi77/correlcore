...
tag _category_, not a slug). The Home work-context heatmap covers the overlap
...
`entry_tags`. The prior backfill was add-only and only _linked_ the 1:1
...
SymptomChecker. Curated default tag _rows_ were never rewritten.
...
| `social`      | Sozial / Social                | skipped (tag _category_, not a slug)     |
