...
This document specifies the **Notes in Analysis** feature for CorrelCore. The goal is to make free-text notes visible across the UI, enrich them with optional structured markers, extract normalized signals from them, and use those signals to power explainable insights.

Milestones M1–M3 are already completed. All work marked **[RETROACTIVE]** must be backported into the existing codebase without breaking current functionality. Work marked **[NEW]** follows the normal sprint cadence from M4 onward.
...
CREATE TABLE entry_note_markers (
  ...
  source      TEXT NOT NULL                    -- 'user' | 'suggestion'
...
Dexie stores `note_raw`, `note_markers[]` (user-sourced only), and a `note_pending_sync: boolean` flag. System signals are computed server-side after sync; the client never stores `entry_note_signals` locally beyond a cache TTL of 24 h.
...
- Chip row below text area showing the predefined marker taxonomy (see below) plus recent user markers.
- Chips are multi-selectable; selected chips are persisted as `entry_note_markers` with `source: 'user'`.
- Character limit: 2000 for `note_raw`, 120 autogenerated for `note_summary_short` (first sentence or truncated).
...
- Days with at least one signal show a second dot (color: `--color-warning`).
- Tooltip on hover / tap: `note_summary_short`.

### Analysis Drilldown — Entry Drawer

- Clicking any data point opens a side drawer showing full entry detail including `note_raw`, markers as chips, and up to 5 top signals with confidence bars.
...
Signal extraction runs as a FastAPI background task (`BackgroundTasks`) triggered on `PATCH /entries/{id}` when `note_raw` is updated.
...
note_raw
  └─► Preprocessing (lowercase, strip HTML)
       └─► Dictionary Lookup       → signals with confidence 0.9
            └─► Regex Pattern Match → signals with confidence 0.6–0.85
                 └─► (Optional, M12+) Local LLM via Ollama
                      → additional signals with confidence 0.4–0.7
```

All extractor rules are version-tagged (`extractor_v`). Re-processing old entries is possible via the operator endpoint without data loss.

### Example Dictionary Rules
...
- `note_visibility = 'hidden'`: note stored but never shown in analysis or insights.
- `note_visibility = 'analysis_only'`: note used in signal extraction and insight generation, but `note_raw` not displayed in shared views or exports.
- `note_visibility = 'full'` (default): full display and analysis.
- Setting is per-entry; a global default can be set in user preferences.
- All signal extraction runs server-side within the selfhosted instance; no external API calls.
- Export (`/export/entries`) includes `note_raw` and signals only when `note_visibility != 'hidden'`.
...
- [ ] Existing entries without notes are unaffected (nullable field).
...
- [ ] No note content is visible on list/card views unless explicitly opened.
...
- [ ] Note section is collapsed by default; expand tap adds < 200ms perceived latency.
