...
| Native `android/` project committed          | Open   |
...
- [ ] Run `cap add android` and **commit** `apps/android/android/` (stop
      gitignoring the platform tree for release builds).
- [ ] Pin Capacitor major versions consistently (`@capacitor/core` /
      `android` / `cli` — resolve current mixed 7/8 pins in `package.json`).
- [ ] Confirm `webDir` (`../web/build/client`) after a production
      `pnpm --filter @correlcore/web build`.
- [ ] App identity: `appId` `de.correlcore.app`, icons, splash, deep-link
      scheme for `/entries/new` (widget later).
- [ ] Document local path: Android Studio / SDK 34+, USB debug install.
- [ ] Extend CI beyond config-only: optional job `assembleDebug` on PRs that
      touch `apps/android/**` (no secrets required).

### Exit

- Debug APK installs on a physical device or emulator and shows the CorrelCore
  UI (even if auth still incomplete).
