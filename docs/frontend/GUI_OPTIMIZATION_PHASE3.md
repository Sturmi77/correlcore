...
- [ ] Mobile `/insights`: user in `robust` with insights never sees `insight-feed-empty` unless API truly returns zero insights
- [ ] Analytics sections visible without user toggle; O-14 maturity gates unchanged
- [ ] Two-row analysis toolbar on Insights; range + filter visually grouped
- [ ] Symptom heatmap/trend charts: no page-level horizontal overflow at 390px with quarter range
- [ ] Symptom calendar has legend + interpretation copy
- [ ] Home: single edit action when today's entry exists; no sparkline; facts row = top insight
- [ ] `OPTIMIZATION_BACKLOG.md` O-43–O-56 marked Done
- [ ] `FRICTION_AUDIT.md` Phase 3 section updated
