...
centres on the hand-maintained FE/BE types.]_ _[2026-09-01: the
CI-automatable external-assurance triad is now complete — SAST via CodeQL
([#801](https://github.com/Sturmi77/correlcore/issues/801)), Trivy container
image scanning (`ci-image-scan.yml`, #802), and DAST via OWASP ZAP against the
live API (`ci-dast.yml`); external assurance now centres on the pending
independent pentest (I1 / #782).]_
...
| Supply chain / CI security | B+                 | B        | gitleaks, pip-audit, pnpm audit, Dependabot, CodeQL SAST, Trivy image scan, ZAP DAST |
...
| I2  | Fixed    | Frontend Vitest coverage floor on `lib/api` + `lib/offline`; backend floor remains 70%                      | [#780](https://github.com/Sturmi77/correlcore/issues/780); `DEVELOPMENT.md` |

### Added 2026-08-25 (gap analysis)

| ID  | Sev    | Status   | Finding                                                                                                                                                                                                      | Location               | Track                                                                                                                          |
| --- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| S1  | Medium | Fixed    | CI-automatable external assurance complete: CodeQL SAST (`codeql.yml`), Trivy image scan (`ci-image-scan.yml`), ZAP DAST vs the live API (`ci-dast.yml`). Independent pentest tracked separately (I1 / #782) | `.github/workflows/`   | [#776](https://github.com/Sturmi77/correlcore/issues/776) (P2 checklist)                                                       |
...
| Q2  | High   | Fixed    | OpenAPI→TS in `packages/api-types` + `ci-contract.yml`; compile-time assertions in `contracts.generated.ts`                                                                                                  | `packages/api-types/`  | [#778](https://github.com/Sturmi77/correlcore/issues/778); `DEVELOPMENT.md`                                                    |
...
| Q5  | Medium | Fixed    | `ci-contract.yml` triggers on backend, `packages/api-types`, FE `lib/contracts` + `lib/api`                                                                                                                  | `ci-contract.yml`      | [#780](https://github.com/Sturmi77/correlcore/issues/780)                                                                      |
...
| P2       | CodeQL SAST ([#801](https://github.com/Sturmi77/correlcore/issues/801)), Trivy image scan (`ci-image-scan.yml`, #802) and ZAP DAST (`ci-dast.yml`) done; remaining: docs hygiene (ADR-0007, OpenAPI version) | [#776](https://github.com/Sturmi77/correlcore/issues/776) checklist |
...
| SAST / DAST / images  | CodeQL or Semgrep + image scan                       | CodeQL SAST (#801) + Trivy image scan (#802) + ZAP DAST + deps audit + gitleaks                              | Covered |
