...
landed — [#801](https://github.com/Sturmi77/correlcore/issues/801) — and Trivy
container image scanning followed (`ci-image-scan.yml`); external assurance now
centres on the pending pentest and DAST.]_
...
| Supply chain / CI security | B+                 | B        | gitleaks, pip-audit, pnpm audit, Dependabot, CodeQL SAST, Trivy image scan; DAST still pending |
...
| --- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| S1  | Medium | Partial  | SAST via CodeQL (`codeql.yml`, python + js/ts, security-extended) and Trivy container image scan (`ci-image-scan.yml`) now in CI; DAST still open | `.github/workflows/`        | [#776](https://github.com/Sturmi77/correlcore/issues/776) (P2 checklist)                                                       |
...
| P2       | CodeQL SAST ([#801](https://github.com/Sturmi77/correlcore/issues/801)) + Trivy image scan (`ci-image-scan.yml`) done; remaining: DAST; docs hygiene (ADR-0007, OpenAPI version) | [#776](https://github.com/Sturmi77/correlcore/issues/776) checklist |
...
| SAST / DAST / images  | CodeQL or Semgrep + image scan                       | CodeQL SAST (#801) + Trivy image scan + deps audit + gitleaks; no DAST yet                                   | Partial |
