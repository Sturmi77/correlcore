...
daily schedule. Only *how* that job is triggered inside the container changes.
...
again — forever. If that in-process loop crashed or hung *inside* a job run,
...
reports the age of the *last successful* run per job, independent of whether
