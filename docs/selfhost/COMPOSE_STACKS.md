...
same as production (#818). There is no separate `digest` profile or container.
Digest **delivery** remains opt-in **per user** (Settings → Analysis,
`digest_enabled`, off by default).
