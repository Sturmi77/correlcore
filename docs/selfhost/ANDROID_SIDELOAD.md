...
> onward; prefer the latest **`v1.x`** release (e.g. **`v1.2.1`**).
...
export ANDROID_VERSION_NAME=1.2.1
export ANDROID_VERSION_CODE=1002001
...
strips the leading `v` for `versionName` (e.g. `v1.2.1` → `1.2.1`) and sets
`versionCode = major×1_000_000 + minor×1_000 + patch` (e.g. `1.2.1` → `1002001`).
...
Tag example: `git tag v1.2.1 && git push origin v1.2.1`.
