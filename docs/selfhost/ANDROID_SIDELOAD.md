...
> onward; prefer the latest **`v1.x`** release (e.g. **`v1.1.8`**).
...
export ANDROID_VERSION_NAME=1.1.8
...
strips the leading `v` for `versionName` (e.g. `v1.1.8` → `1.1.8`) and sets
`versionCode = major×1_000_000 + minor×1_000 + patch` (e.g. `1.1.8` → `1001008`).
...
Tag example: `git tag v1.1.8 && git push origin v1.1.8`.
