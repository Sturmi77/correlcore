...
IMAGE_TAG=v1.7.1                  # any v1.x pin works; or sha-<short> from GHCR / Docker Hub
```
...
> **Upgrading from a stack that ran `COMPOSE_PROFILES=…,digest`:** the weekly
> digest is now generated by the analytics `worker`, and the standalone
> `digest-worker` service no longer exists. A plain `docker compose up -d`
> leaves the old container as an orphan (it keeps running on the stale image and
> would write duplicate digest snapshots). If the **compose file on disk still
> defines** `digest-worker` and you pull a new API image, that leftover service
> is not an orphan — it would be recreated. Remove it once after upgrading — e.g.
> `docker compose rm -sf digest-worker`, or bring the stack up with
> `docker compose up -d --remove-orphans` after replacing the compose files —
> and drop `digest` from `COMPOSE_PROFILES`.
