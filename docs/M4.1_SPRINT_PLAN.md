...
- [ ] `docs/adr/0036-offline-sync-v1-scope.md` (or extend ADR-0003 addendum)
  - Entity list, field merge rules, cursor format, idempotency keys
  - Explicit: encrypted `note_enc` conflict log stores no plaintext
- [ ] `docs/M4.1_SPRINT_STATUS.md` tracking file (this plan's companion)
- [ ] `API.md` §10: finalize request/response JSON schemas and error codes (`409` vs conflict report)
- [ ] Dexie ERD diagram in plan or ADR: tables `entries_local`, `change_log`, `sync_meta`
- [ ] OpenAPI stubs or `backend/app/schemas/sync.py` skeleton (no endpoints yet)
...
- [ ] Alembic migration: `sync_conflicts` per ADR-0003 / #24
- [ ] SQLAlchemy model `SyncConflict`
- [ ] `GET /api/v1/user/sync-conflicts` — paginated, `entity_type` filter, no plaintext health values in response
- [ ] APScheduler job: delete rows older than 90 days
- [ ] `docs/API.md` — conflict list endpoint documented

### Tests

- [ ] Pytest: migration smoke (CI alembic upgrade)
- [ ] Pytest: create/list conflict rows; retention job deletes stale rows
- [ ] Pytest: response schema never includes decrypted note text
...
- [ ] `backend/app/schemas/sync.py` — `SyncPushRequest`, `SyncPullResponse`, `SyncChange`, `SyncConflictReport`
...
- [ ] `POST /api/v1/sync/push` — authenticated, idempotent on `change_log` sequence + `client_id`
- [ ] `GET /api/v1/sync/pull?since=<cursor>` — delta since cursor; includes entries + custom tags/symptoms
- [ ] Router mount in `api/v1/router.py` (replace future comment)
- [ ] Structured logging: sync batch size, conflict count — **no** mood/note values

### Tests

- [ ] Pytest: push new entry → visible in DB
- [ ] Pytest: pull returns only changes after cursor
- [ ] Pytest: two devices edit same field → conflict row + server value wins
- [ ] Pytest: push idempotency (replay same batch)
- [ ] Pytest: RLS / user isolation (user A cannot pull user B)
...
- [ ] Add `dexie` dependency to `apps/web`
- [ ] `apps/web/src/lib/offline/db.ts` — Dexie database version 1
- [ ] Tables: `entries`, `change_log`, `sync_meta` (`client_id`, `last_pull_cursor`)
- [ ] `apps/web/src/lib/offline/clientId.ts` — stable UUID in `localStorage`
- [ ] `apps/web/src/lib/offline/changeLog.ts` — append-only outbox with monotone sequence
- [ ] Unit tests: round-trip write/read; sequence monotonicity

### Tests

- [ ] Vitest with `fake-indexeddb` (or jsdom IndexedDB shim)
- [ ] No change to production entry routes yet (feature flag `offlineSyncEnabled` default off until Sprint 4)
...
- [ ] `apps/web/src/lib/offline/syncOrchestrator.ts`
  - `pushPending()`, `pullSince(cursor)`, `onOnline` / `visibilitychange` hooks
  - Integrate with existing `pwaLifecycle` store
- [ ] `apps/web/src/lib/stores/entriesOffline.ts` (or extend `entries` store)
  - Local-first write path for `/entries` day form
  - Optimistic UI; `SaveStatusBadge` states: `local` | `syncing` | `synced` | `offline`
- [ ] Replace in-tab-only offline retry with Dexie-backed queue where feature flag on
- [ ] Handle `push` conflict reports — surface neutral copy (no alarmist gamification)
- [ ] Settings > App & offline: show last sync time + pending change count
- [ ] Feature flag: enable offline sync for verified users (env or preference)

### Tests

- [ ] Vitest: orchestrator push/pull mocked fetch
- [ ] E2E: `offline entry → reload → online → entry on server` (extend `mobile-entry-foundation.spec.ts`)
- [ ] E2E: existing autosave tests still pass when flag off
...
- [ ] `docs/quality/M4.1_VISUAL_QA.md` — 375 / 768 / 1280, light + dark
  - Offline entry form, pending badge, reconnect sync, settings sync summary
- [ ] `docs/M4.1_SPRINT_STATUS.md` — all sprints Done
- [ ] `docs/features/PWA.md` — offline-first section updated
- [ ] `CHANGELOG.md` — M4.1 entry
- [ ] `README.md` — Offline Sync row updated from „Planned" to „Complete" or „Partial"
...
| ---------------------------------------------------- | ---------- | ---- |
| Create entry offline, reload, go online              | Phone PWA  |      |
| Edit same entry on phone + desktop                   | 2 browsers |      |
| Conflict on `mood_score` → row in sync-conflicts API | 2 browsers |      |
| Airplane mode → banner → retry → synced              | Phone      |      |
