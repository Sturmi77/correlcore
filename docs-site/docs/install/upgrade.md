...
release they trust (e.g. **`v1.1.1`**; any **`v1.x`** pin works), then pull and
restart:

```bash
cd correlcore/infra/docker

# Set in .env, e.g. IMAGE_TAG=v1.1.1
grep IMAGE_TAG .env
...
Optional: also set `APP_VERSION=1.1.1` so `/api/v1/health` and error-tracking
