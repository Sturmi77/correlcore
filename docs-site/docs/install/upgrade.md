...
| Who                  | Action                                                              |
| -------------------- | ------------------------------------------------------------------- |
| Production / homelab | Pin `IMAGE_TAG=v1.9.1`, pull, `up -d --remove-orphans`              |
| `.env`               | **No required new vars.**                                           |
| Database             | No new Alembic revision beyond **046** (already required for 1.9.0) |
...
curl -sf "https://${DOMAIN}/api/v1/health"   # "version":"1.9.1"
```

Rollback: set `IMAGE_TAG=v1.9.0` and `up -d`.

---

## v1.9.0

From **v1.8.0** to **v1.9.0**. Feature minor — Home-Screen additions: „Auf einen
Blick" trends summary section, 28-day trend indicators on the weekday and
work-context cards, sortable work-situation map, and worker run-duration in the
analysis badge and Dev/Workers. No new required env vars.

| Who                  | Action                                                                                      |
| -------------------- | ------------------------------------------------------------------------------------------- |
| Production / homelab | Prefer `IMAGE_TAG=v1.9.1`; or pin `v1.9.0`, pull, `up -d --remove-orphans`                  |
| `.env`               | **No required new vars.**                                                                   |
| Database             | `migrate` applies Alembic through **046** (`home_weekday_day_trend_enabled`) if not already |
...
Rollback from 1.9.0: set `IMAGE_TAG=v1.8.0` and `up -d`.

---

## v1.8.0

From **v1.7.1** to **v1.8.0**. Feature minor — Trends data-readiness panel, Home
analysis badge, Developer-View stack health, and P-balanced insights. No new
required env vars.

| Who                  | Action                                                                               |
| -------------------- | ------------------------------------------------------------------------------------ |
| Production / homelab | Pin `IMAGE_TAG=v1.8.0`, pull, `up -d --remove-orphans`                               |
| `.env`               | **No required new vars.**                                                            |
| Database             | `migrate` applies Alembic through **045** (`collapse_lag_dismissals`) if not already |
...
docker compose pull
docker compose up -d --remove-orphans
curl -sf "https://${DOMAIN}/api/v1/health"   # "version":"1.8.0"
```

Rollback: set `IMAGE_TAG=v1.7.1` and `up -d`.
...
**v1.9.1** after those checks). Skipping those steps can still prevent the API
from starting or return **415** to custom clients.

| Who                  | Action                                                                                                                                    |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Production / homelab | Prefer `IMAGE_TAG=v1.9.1`; or pin `v1.7.0`, pull, `up -d --remove-orphans` (run [v1.6.0](#v160) blockers first if upgrading from earlier) |
