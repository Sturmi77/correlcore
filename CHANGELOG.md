...
- **Container-Image-Builds zum ersten Mal lauffähig.** Beide Dockerfiles waren zwar im Repo, aber kein Image war je gebaut/gepublished worden — was beim ersten Run des neuen `release-images.yml`-Workflows (PR #76 / Commit 4378fcb) sichtbar wurde.
  ...
  - **`apps/web/Dockerfile`**: drei-Stage-Refactor nötig, weil mehrere pnpm-Quirks in Folge zugeschlagen haben — `pnpm prune --prod` hängte mit "confirmModulesPurge"-Prompt im non-TTY-Container; ein Fix mit `pnpm install --prod` nach dem Build kippte mit `ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY` (`CI=true` half nicht); ein weiterer Versuch mit `ENV CI=true` legte das eigentliche Problem frei: das `prepare`-Script in `apps/web/package.json` ruft `svelte-kit sync` auf, das im `--prod`-Modus nicht verfügbar ist (`sh: svelte-kit: not found`). Finaler Fix: separater **`deps`-Stage** mit `pnpm install --prod --frozen-lockfile --ignore-scripts`, der die Prod-`node_modules` isoliert produziert; `builder`-Stage baut mit allen Deps; Runtime-Stage kombiniert `build/` aus builder + `node_modules/` aus deps. `ENV CI=true` bleibt drin als Defense-in-Depth gegen weitere TTY-Prompts.
